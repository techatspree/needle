...
class DatabaseTestcaseConfiguration {
  ...
  DatabaseTestcaseConfiguration(final Class<?>... clazzes) {
    this();
    assert clazzes != null : "clazzes != null";
    this.entityClazzes = clazzes;
  }

  DatabaseTestcaseConfiguration(final String persistenceUnitName) {
    this();
    assert persistenceUnitName != null : "persistenceUnitName != null";
  ...
  private AbstractDBOperation createDBOperation() {
    ...
    if (dbOperationClass != null) {
      try {
        ...
        LOG.error("could not create a new instance of db dialect " + dbOperationClass, e);
      }
    } else {
  ...
  private JdbcConfiguration getEntityManagerFactoryProperties() throws Exception {
    try {
      ...
      return new JdbcConfiguration((String) properties.get(JDBC_URL_KEY), (String) properties.get(JDBC_DRIVER_KEY),
          (String) properties.get(JDBC_USER_KEY), (String) properties.get(JDBC_PASSWORD_KEY));
    } catch (final Exception e) {
